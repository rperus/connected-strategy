...
interface FrontendPerfInput {
  files: DiscoveredFile[];
}

export function registerFrontendPerf(hub: EventHub, ctx: any): void {
    hub.subscribe<FrontendPerfInput>('_R_U_N__FRONTEND_PERF', async (event) => {
          const input = event.payload;
          const start = Date.now();
      try {
        const relevantFiles = input.files.filter(f => f.category === 'component');
        const filesContent = relevantFiles.map(f => `--- ${f.relativePath} ---\n${ctx.fileReader.read(f.relativePath)}`).join('\n\n');

        const prompt = `
Actúa como Frontend Performance Expert.
Revisa los componentes frontend buscando renderizados innecesarios, bundle bloat o problemas de accesibilidad.
...
`;
        ...
        await hub.publish({
          domain: 'lifecycle',
          type: '_R_U_N__FRONTEND_PERF_COMPLETED',
        ...
        await hub.publish({
          domain: 'lifecycle',
          type: '_R_U_N__FRONTEND_PERF_COMPLETED_FAILED',
          projectId: event.projectId,
